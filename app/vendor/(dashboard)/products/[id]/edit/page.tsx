...
export default function EditProduct() {
  ...
  useEffect(() => {
    // Fetch categories and the product in parallel
    Promise.all([
      fetch("/api/categories").then((r) => r.json()),
      fetch(`/api/vendor/products/${id}`).then((r) => r.json()),
    ])
      .then(([catData, productData]) => {
        ...
        if (productData.product) {
          ...
          setUploadedImages(Array.isArray(p.images) ? p.images : JSON.parse(p.images || "[]"));
        } else {
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="w-8 h-8 border-4 border-[#0b8241] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
